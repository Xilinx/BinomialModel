...
int main(int argc, char* argv[])
{
	...
	typedef struct {
			...
			t_in_data*       host_IBuf[2];          // In Buffer in Host Mem associated with a kernel

			cl_mem           GlobMem_IBuf[2];       // In Buffer in Global Mem associated with a kernel
			cl_mem_ext_ptr_t GlobMem_IBuf_EXT[2];
			cl_mem           GlobMem_OBuf[2];       // OUT Buffer in Global Mem associated with a kernel
			cl_mem_ext_ptr_t GlobMem_OBuf_EXT[2];

			float*           host_OBuf[2];          // OUT Buffer in Host Mem associated with a kernel
	} t_kernel;
	...
	for (int i=0; i<(SW_HW_Config).NB_OF_KERNELS; i++) {
		...
		//............................................................
		HW_Kernels[i].host_IBuf[0] = allocate_host_mem<t_in_data>(HW_Kernels[i].Nb_Of_Test_Vectors,HW_Kernels[i].name+".host_IBuf[0]",true);
		HW_Kernels[i].host_OBuf[0] = allocate_host_mem<float>(HW_Kernels[i].Nb_Of_Test_Vectors,HW_Kernels[i].name+".host_OBuf[0]",true);

		HW_Kernels[i].host_IBuf[1] = allocate_host_mem<t_in_data>(HW_Kernels[i].Nb_Of_Test_Vectors,HW_Kernels[i].name+".host_IBuf[1]",true);
		HW_Kernels[i].host_OBuf[1] = allocate_host_mem<float>(HW_Kernels[i].Nb_Of_Test_Vectors,HW_Kernels[i].name+".host_OBuf[1]",true);
	}

	for (int i=0; i<(SW_HW_Config).NB_OF_KERNELS; i++) {
		for (int buff_nb=0; buff_nb<2; buff_nb++) {

			// GlobMem_IBuf
			// .....................
			cout << "HOST-Info: Allocating Global Memory for " + HW_Kernels[i].name+".GlobMem_IBuf[" + to_string(buff_nb) + "]" << endl;
			HW_Kernels[i].GlobMem_IBuf[buff_nb] = clCreateBuffer(Context, CL_MEM_READ_ONLY | CL_MEM_USE_HOST_PTR, HW_Kernels[i].Nb_Of_Test_Vectors * sizeof(t_in_data),  HW_Kernels[i].host_IBuf[buff_nb], &errCode);
			ocl_check_status(errCode,"Failed to allocate Global Memory for " + HW_Kernels[i].name+".GlobMem_IBuf[" + to_string(buff_nb) + "]");

			errCode = clEnqueueMigrateMemObjects(Command_Queue, 1, &(HW_Kernels[i].GlobMem_IBuf[buff_nb]), CL_MIGRATE_MEM_OBJECT_CONTENT_UNDEFINED, 0, NULL, NULL);
			ocl_check_status(errCode,"Failed to Migrate " + HW_Kernels[i].name + ".GlobMem_IBuf[" + to_string(buff_nb) + "] from Host Memory");


			// GlobMem_OBuf
			// .....................
			cout << "HOST-Info: Allocating Global Memory for " + HW_Kernels[i].name+".GlobMem_OBuf[" + to_string(buff_nb) + "]" << endl;
			HW_Kernels[i].GlobMem_OBuf[buff_nb] = clCreateBuffer(Context, CL_MEM_WRITE_ONLY | CL_MEM_USE_HOST_PTR, HW_Kernels[i].Nb_Of_Test_Vectors * sizeof(float), HW_Kernels[i].host_OBuf[buff_nb], &errCode);
			ocl_check_status(errCode,"Failed to allocate Global Memory for " + HW_Kernels[i].name+".GlobMem_OBuf[" + to_string(buff_nb) + "]");

			errCode = clEnqueueMigrateMemObjects(Command_Queue, 1, &(HW_Kernels[i].GlobMem_OBuf[buff_nb]), CL_MIGRATE_MEM_OBJECT_CONTENT_UNDEFINED, 0, NULL, NULL);
			ocl_check_status(errCode,"Failed to Migrate " + HW_Kernels[i].name + ".GlobMem_OBuf[" + to_string(buff_nb) + "] from Host Memory");
	...
	for (int i=0; i<ROUNDED_NB_OF_TESTS; i++) {
		...
		if (i<2) {
			HW_Kernels[k_index].host_IBuf[buff_nb][0] = host_IN_DATA[i];
		} else { // (i >= 2)
			clWaitForEvents(1,&Mem_rd_event[i-2]);
			hw_RES[i-2] = HW_Kernels[k_index].host_OBuf[buff_nb][0];
			HW_Kernels[k_index].host_IBuf[buff_nb][0] = host_IN_DATA[i];
		}
		...
		// .................................................................
	    errCode = clEnqueueMigrateMemObjects(Command_Queue, 1, &(HW_Kernels[k_index].GlobMem_IBuf[buff_nb]), 0,0, NULL, &Mem_wr_event[i]);
		...
		int arg_indx = 0;
		errCode = CL_SUCCESS;
		errCode |= clSetKernelArg(HW_Kernels[k_index].kernel,  arg_indx++, sizeof(cl_mem),    &(HW_Kernels[k_index].GlobMem_IBuf[buff_nb]));
		errCode |= clSetKernelArg(HW_Kernels[k_index].kernel,  arg_indx++, sizeof(cl_mem),    &(HW_Kernels[k_index].GlobMem_OBuf[buff_nb]));
		...
		// .................................................................
		errCode = clEnqueueMigrateMemObjects(Command_Queue, 1, &(HW_Kernels[k_index].GlobMem_OBuf[buff_nb]), CL_MIGRATE_MEM_OBJECT_HOST,
	...
	// ----------------------------------------------
	// Write two last pending results from execution
	// ----------------------------------------------
	for (int i=ROUNDED_NB_OF_TESTS-2; i<ROUNDED_NB_OF_TESTS; i++) {
		int buff_nb = i % 2;
		clWaitForEvents(1,&Mem_rd_event[i]);
		hw_RES[i] = HW_Kernels[k_index].host_OBuf[buff_nb][0];
	}
	...
	for (int i=0; i<(SW_HW_Config).NB_OF_KERNELS; i++) {
		clReleaseMemObject(HW_Kernels[i].GlobMem_IBuf[0]);
		clReleaseMemObject(HW_Kernels[i].GlobMem_OBuf[0]);
		clReleaseMemObject(HW_Kernels[i].GlobMem_IBuf[1]);
		clReleaseMemObject(HW_Kernels[i].GlobMem_OBuf[1]);
	}
